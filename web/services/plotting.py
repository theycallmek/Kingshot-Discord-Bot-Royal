...
def generate_town_center_graphs(
    users: List[Any],
    furnace_changes: List[Any],
    all_alliances: List[Any],
    alliance_nicknames: Dict[str, str],
) -> Dict[str, str]:
    """
    ...
    daily_alliance_stats = defaultdict(
    ...
    colors = {
        aid: color_palette[i % len(color_palette)]
        for i, aid in enumerate(target_alliances)
    }
    alliance_names = {
        aid: alliance_nicknames.get(aid, f"Alliance {aid}") for aid in target_alliances
    }
    ...
    for alliance in target_alliances:
        dates = [
            ...
            if alliance in daily_alliance_totals[d]
        ]
        totals = [
            daily_alliance_totals[d][alliance]
            for d in sorted_dates
            if alliance in daily_alliance_totals[d]
        ]
        if dates:
        ...
        row_data = [
            sum(
                ...
                if (
                    ...
                    and user.furnace_lv
                    and min_tc <= user.furnace_lv <= max_tc
                )
            )
    ...
    heatmap = go.Heatmap(
        ...
        colorbar=dict(
            title=dict(text="Members", side="right"),
            tickmode="linear",
            tick0=0,
            dtick=1,
        ),
...
def generate_bear_trap_graph(bear_trap_data: List[Any]) -> str:
    """
    Generates the Bear Trap damage dealers bar chart.

    Args:
        bear_trap_data: A list of OCREventData model objects.
    ...
    sorted_players = sorted(
        player_damage.items(), key=lambda item: item[1], reverse=True
    )[:15]
    ...
    bar = go.Bar(
        x=player_names,
        y=damage_values,
        marker=dict(color=damage_values, colorscale="Viridis", showscale=True),
        hovertemplate="<b>%{x}</b><br>Damage: %{y}<extra></extra>",
    )
    fig_bear_trap = go.Figure(data=[bar])
