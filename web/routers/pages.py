...
from web.core.database import (
    get_users_session,
    get_changes_session,
    get_alliance_session,
    get_cache_session,
    get_beartime_session,
    get_giftcode_session,
)
...
async def read_events(
    ...
    statement = select(BearNotification).options(
        selectinload(BearNotification.embeds),
        selectinload(BearNotification.notification_days),
    )
...
async def read_giftcodes(
    ...
    return templates.TemplateResponse(
        "giftcodes.html",
        {"request": request, "all_codes": all_codes, "redemption_map": redemption_map},
    )
...
async def read_logs(
    ...
    try:
        nickname_changes.sort(
            key=lambda x: datetime.fromisoformat(x.change_date), reverse=True
        )
        furnace_changes.sort(
            key=lambda x: datetime.fromisoformat(x.change_date), reverse=True
        )
...
async def read_data(
    ...
    tc_graphs = generate_town_center_graphs(
        users, furnace_changes, all_alliances, alliance_nicknames
    )
    ...
    alliance_names = [
        alliance_nicknames.get(aid, f"Alliance {aid}") for aid in alliance_counts.keys()
    ]
    ...
    graph_bear_trap_top_json = json.dumps(
        fig_bear_trap_top, cls=plotly.utils.PlotlyJSONEncoder
    )
