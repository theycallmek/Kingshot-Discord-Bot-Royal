...
# Connection arguments for all databases
sqlite_connect_args = {"check_same_thread": False}

users_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'users.sqlite')}",
    connect_args=sqlite_connect_args,
)
giftcode_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'giftcode.sqlite')}",
    connect_args=sqlite_connect_args,
)
changes_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'changes.sqlite')}",
    connect_args=sqlite_connect_args,
)
attendance_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'attendance.sqlite')}",
    connect_args=sqlite_connect_args,
)
beartime_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'beartime.sqlite')}",
    connect_args=sqlite_connect_args,
)
alliance_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'alliance.sqlite')}",
    connect_args=sqlite_connect_args,
)
cache_engine = create_engine(
    f"sqlite:///{os.path.join(DB_DIR, 'web_cache.sqlite')}",
    connect_args=sqlite_connect_args,
)
